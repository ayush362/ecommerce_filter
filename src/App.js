...
function App() {
  ...
  console.log("Render");
  console.log(filters.color, filters.company);
  ...
  return (
    <div className="grid grid-cols-3 gap-5 p-10">
      {products
        ...
        .filter((p) =>
          p.newPrice.toLowerCase().includes(filters.price.toLowerCase())
        )
        ...
        .map((product) => (
          <div className="flex flex-col border rounded-lg shadow">
            <img
              src={product.img}
              alt={product.title}
              className=" h-48 w-full object-contain bg-white p-5"
            />
            <div className="p-4">
...
const ColorFilter = ({ filters, setFilters }) => {
  console.log("Rendered color filter");
...
const PriceFilter = ({ filters, setFilters }) => {
  const prices = [...new Set(products.map((p) => p.newPrice))];
  return (
    <div className="flex flex-col gap-1 border p-2">
      <h2>Prices</h2>
      <select
